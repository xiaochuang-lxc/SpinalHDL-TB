...
case class AxiLite4WSource(bus: Stream[AxiLite4W], clockDomain: ClockDomain, queueOccupancyLimit: Int = 8) extends StreamSource(bus, clockDomain, queueOccupancyLimit) {

  var flowPercent: Int = 100

  override def sendData[T <: Transaction](transaction: T): Unit = {
    val pkg = transaction.asInstanceOf[AxiLite4WPkg]
    bus.data #= pkg.data
    bus.strb #= pkg.strb
  }
