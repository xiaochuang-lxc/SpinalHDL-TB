...
class MemoryRegionTest extends FunSuite {
  ...
  test("normal read and write match") {
    ...
    val readDwords = memoryRegionInst.readDords(0x10, writeDwords.length)
  ...
  test("window read and write") {
    ...
    val readDowrds1 = window.readDords(0x10, writeDwords.length)
  ...
  test("window pool read and write") {
    ...
    def windowTest(window: Window, address: BigInt) = {
      ...
      val readDwords0 = window.readDords(address, writeDwords.length)
      assert(writeDwords.toBuffer == readDwords0.toBuffer, s"writeBytes:$writeDwords\nreadBytes:$readDwords0")
      val readDwords1 = windowPool.readDords(window.base - memoryRegionInst.base + address, writeDwords.length)
      assert(writeDwords.toBuffer == readDwords1.toBuffer, s"writeBytes:$writeDwords\nreadBytes:$readDwords1")
      val readDwords2 = memoryRegionInst.readDords(windowPool.base + window.base - memoryRegionInst.base + address, writeDwords.length)
